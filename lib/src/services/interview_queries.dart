...
class InterviewQueries {
  ...
  Future<List<InterviewDetails>> getInterviewDetails(int dummy) async {
    //await initDatabase();
    ...
    String dbQuery =
        '''Select residentDetails.dhamma_name, residentDetails.kuti, residentDetails.country, interviews.stime 
          ...
          WHERE residentDetails.id_code = interviews.id_code AND stime = '${now.day}/${now.month}/${now.year}'
  ...
  Future addInterviewRecord(String iDCode) async {
    final _db = await _dbHelper.database;
    final now = DateTime.now();

    String dbQuery =
        '''Insert INTO interviews (id_code, stime, real_time, teacher) 
        SELECT '$iDCode','${now.day}/${now.month}/${now.year}',${now.microsecondsSinceEpoch},'1' 
        WHERE NOT EXISTS(
                         SELECT id_code, stime FROM interviews 
                        WHERE id_code = '$iDCode' AND stime = '${now.day}/${now.month}/${now.year}'
                        );''';
